...
The results of a DTA are usually a list of genes, each categorized into a different *Regulatory Mode* depending on the changes in mRNA levels and TE between the two conditions. Such an output, although informative per se, but is also be quite complex to interpret when it comes to link together the different genes to infer the overarching metabolic shifts happening in the experiment.
...
While ***terapadog*** can be run as a tool on its own (as shown in this vignette), it was originally conceived to be an additional analysis method to the ReactomeGSA package, so to leverage on the visualisation methods and tools offered by Reactome.

That is to say, while ***terapadog*** on its own allows the user to study genes that are undergoing changes in their TE and the pathways they are involved in, it is in its integration with the ReactomeGSA that it truly shines, thanks to the advanced framework and user-friendly visualization.
...
a small report .txt file is produced in the working directory, allowing the user 
...
Since running terapadog can be computationally intesive, only a non-runnable 
example of the code will be provided:
...
This functions perform the Differential Translation Analysis (DTA) on its own.
...
plot_FCs <- plotDTA(fc_results, paste0(getwd(), "/manual_example.html"))
```

# References

Terapadog has ben developed starting from the PADOG  algorithm of anaylisis.
I do not own the original, just the idea of implementing it on top of DeltaTE 
for the analysis of translational changes.

Tarca AL, Draghici S, Bhatti G. et al. Down-weighting overlapping genes improves gene set analysis. BMC Bioinformatics (2012).

[<https://doi.org/10.1186/1471-2105-13-136>]

Similarly, the idea and data that have been used to test the package are from:
...
Specifically, DeltaTE it makes use of the  lfcShrink from the apeglm package:
...
Finally, I report the ReactomeGSA, since this tool has been developed with the 
idea in mind to have it integrated with it once published.
